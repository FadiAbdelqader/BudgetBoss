...
<script>
  export default {
    ...
    parseHoraireFermeture : function(horaireObj) {
      ...
      fetchPrix() {
        fetch(`http://localhost:3004/api/comparator/carburants/prix/${this.region}/${this.carburant}`)
      ...
      fetchAllPrix() {
        fetch(`http://localhost:3004/api/comparator/carburants/allprix/${this.region}/${this.carburant}`)
          .then((response) => response.json())
          .then((data) => {
            this.prixList = data
            this.prixB = false

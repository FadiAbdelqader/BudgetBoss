<template>
    <div>
      <div class="titre_boutons">
        <h2 font-weight: bold>Stations en {{ region }} - {{ carburant }} :</h2>
        <div class="boutons">
          <div class="button_prix">
            <button @click="fetchPrix">Station moins chère</button>
          </div>
          <div class="button_allprix">
            <button @click="fetchAllPrix">Toutes les stations</button>
          </div>
      </div>
    </div>
      
      <div class = "station" v-if="prix && prixB && !allprixB">
        <p font-weight: bold><strong>Adresse:</strong> {{ prix.adresse }}, {{ prix.ville }}, {{ prix.departement }}</p>
        <p font-weight: bold v-if="prix.horaires_automate_24_24 === 'Oui'">Auto ouvert 24h/24 disponible dans cette station</p>
        <p font-weight: bold v-if="prix.horaires_automate_24_24 === 'Non'">Auto ouvert 24h/24 indisponible dans cette station</p>
        <div v-if="prix.prix">
          <p font-weight: bold >Prix unitaire : {{ prix.prix }} €</p>
        </div>
        <div v-else>
          <p font-weight: bold >Prix unitaire : indisponible</p>
        </div>
        ...
        <ul>
          ...
          <li font-weight: bold v-else>Aucun service disponible</li>
        </ul>
        <p font-weight: bold><strong>Horaires ouverture semaine :</strong></p>
        <ul>
          <li v-if="prix.horaires">
            ...
            <div v-else>
              <p font-weight: bold>Horaires indisponibles</p>
            </div>
          </li>
          <li v-else>
            <p font-weight: bold>Horaires indisponibles</p>
      ...
      <div class="station-container" v-if="prixList && !prixB && allprixB" >
        <div class="station" v-for="prix in prixList">
          <p font-weight: bold>Adresse : {{ prix.adresse }}, {{ prix.ville }}, {{ prix.departement }}</p>
          ...
          <p font-weight: bold>Services disponibles dans cette station : </p>
          <ul>
            <li v-if="prix.services">
              <div v-for="(service, index) in JSON.parse(prix.services).service" :key="index">
                {{ service }}
          ...
          <p font-weight: bold>Horaires ouverture semaine : </p>
          <ul>
            <li v-if="prix.horaires">
              <div v-if="JSON.parse(prix.horaires).jour">
                <div v-for="(jour, index) in JSON.parse(prix.horaires).jour" :key="index">
                  ...
                  <div v-else>
                    <p font-weight: bold>{{jour['@nom']}} : indisponible </p>
                  </div>
                </div>
              </div>  
              <div v-else>
...
<style>
@import "../../assets/Comparator/results.css";
</style>
...
<script>
  export default {
    ...
    parseHoraireFermeture : function(horaireObj) {
      ...
      fetchAllPrix() {
        fetch(`http://localhost:2001/compare/allprix/${this.region}/${this.carburant}`)
          .then((response) => response.json())
          .then((data) => {
            this.prixList = data
            this.prixB = false

<template>
  <dev width="auto">
    <h1>To-Buy List</h1>
    <div class="menu">
      <p class="tasks-left-text">
        {{ incompletedTodos.length }} task(s) left! / {{ todos.length }}
      </p>
    ...
    <div class="parentSelectInput">
      ...
      <div>
        ...
        <div v-if="!tobuyObject.category">Please select an option.</div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async removeTodo(todo) {
      try {
        if (todo.completed) {
          // Create a new expense based on the to-buy item being removed
          const expense = {
            name: todo.name,
            amount: 0, // Set the amount to 0 for now, as it is not specified in the to-buy list
            category: "hgfkjhglg",
          };
